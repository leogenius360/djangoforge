"""Tests for authn exception handler."""
...
def _data(response):
    """Extract JSON data from a JsonResponse."""
    return json.loads(response.content)


@pytest.fixture
def context():
    """Minimal exception handler context."""
...
class TestAuthnExceptionHandler:
    ...
    def test_base_authentication_error(self, context):
        ...
        assert _data(response)["detail"] == "An authentication error occurred."
        assert _data(response)["error_code"] == "AuthenticationError"

    def test_invalid_credentials_error(self, context):
        exc = InvalidCredentialsError("bad password")
        response = authn_exception_handler(exc, context)
        assert response.status_code == 401
        assert _data(response)["detail"] == "Invalid credentials."
        assert _data(response)["error_code"] == "InvalidCredentialsError"

    def test_credential_expired_error(self, context):
        exc = CredentialExpiredError()
        response = authn_exception_handler(exc, context)
        assert response.status_code == 401
        assert _data(response)["detail"] == "Credentials have expired."

    def test_password_reuse_error(self, context):
        exc = PasswordReuseError()
        response = authn_exception_handler(exc, context)
        assert response.status_code == 400
        assert _data(response)["detail"] == "Cannot reuse a recent password."
    ...
    def test_password_validation_error(self, context):
        ...
        response = authn_exception_handler(exc, context)
        assert response.status_code == 400
        assert _data(response)["detail"] == "Password does not meet requirements."
        assert _data(response)["validation_errors"] == errors
    ...
    def test_password_validation_error_no_errors(self, context):
        ...
        assert "validation_errors" not in _data(response)
    ...
    def test_cooldown_active_error(self, context):
        ...
        response = authn_exception_handler(exc, context)
        assert response.status_code == 429
        assert _data(response)["retry_after"] == 60
    ...
    def test_error_code_matches_class_name(self, context):
        ...
        for exc in exceptions:
            response = authn_exception_handler(exc, context)
            assert _data(response)["error_code"] == type(exc).__name__
    ...
    def test_internal_detail_not_leaked(self, context):
        ...
        assert "xyz" not in _data(response)["detail"]
        assert "mismatch" not in _data(response)["detail"]
